...
The calendar covers GitHub's rolling contribution year. Public repositories exclude forks and repositories owned by other accounts. Pull requests are public authored PRs across all time. Source language proportions exclude Jupyter Notebook bytes before ranking and calculating percentages, with the top three and Other shown. The raw snapshot retains the API's language data; the renderer applies the exclusion on every refresh.
